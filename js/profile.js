    ...
    .then((data) => {
      ...
      for (var i = 0; i < RES.length; i++) {
        if (verdictMap.has(RES[i].verdict))
          verdictMap.set(RES[i].verdict, verdictMap.get(RES[i].verdict) + 1);
      ...
      for (let [key, value] of heatmapMap) {
        ...
        let temp = key.split("-");
        heatmap.push([
          new Date(parseInt(temp[0]), parseInt(temp[1]), parseInt(temp[2])),
          value,
        ]);
      }
      // console.log(heatmap);
      ...
      document.querySelector(".heatmap").style.height = `${heiOfHeatmap}em`;
