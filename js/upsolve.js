...
async function showUpsolveProblems(conLis) {
  for (var i = 0; i < Math.min(conLis.length, 20); i++) {
    ...
    await delay(1000);
    fetch(
      `${apiURL}contest.status?contestId=${obj.id}&from=1&count=1000&handle=${CFHandleVal}`
    )
      .then((res) => {
        if (res.ok) {
    ...
    await delay(1000);
    fetch(
      `${apiURL}contest.standings?contestId=${obj.id}&from=1&count=2&showUnofficial=true`
    )
      .then((res) => {
        if (res.ok) {
...
function howManyTillNow() {
  var cnt = 0;
  if (card_1up.style.display === "flex") {
    cnt++;
  }
  if (card_2up.style.display === "flex") {
    cnt++;
  }
  if (card_3up.style.display === "flex") {
    cnt++;
  }
  if (card_4up.style.display === "flex") {
    cnt++;
  }
  if (card_5up.style.display === "flex") {
    cnt++;
  }
  return cnt;
}
  ...
  for (var i = 0; i < probArray.length; i++) {
    ...
    if (obj.verdict != "OK") {
      ...
      for (var j = 0; j < i; j++) {
        ...
        if (
          obj.problem.name === objInner.problem.name &&
          objInner.verdict == "OK"
        ) {
          possible = false;
          break;
